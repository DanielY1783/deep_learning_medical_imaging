...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        # Two more convolutional layers before maxpooling
        self.conv9 = nn.Conv2d(120, 240, 3, 1)
        self.conv9_bn = nn.BatchNorm2d(240)
        self.conv10 = nn.Conv2d(240, 240, 3, 1)
        self.conv10_bn = nn.BatchNorm2d(240)
    ...
    def forward(self, x):
        ...
        # Output dimensions: 25x15x240
        x = self.conv9(x)
        x = self.conv9_bn(x)
        x = F.relu(x)
        x = self.dropout1(x)
        # Input dimensions: 25x15x240
        # Output dimensions: 23x13x240
        x = self.conv10(x)
        x = self.conv10_bn(x)
        x = F.relu(x)
        x = self.dropout1(x)
        # Input dimensions: 23x13x240
        # Output dimensions: 12x7x240
        x = F.max_pool2d(x, 2, ceil_mode=True)


        # Input dimensions: 27x17x240
        # Output dimensions: 20160x1
        x = torch.flatten(x, 1)
        # Input dimensions: 20160x1
