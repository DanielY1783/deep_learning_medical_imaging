...
# Constants
MODEL_NAME = "/content/drive/My Drive/cs8395_deep_learning/assignment3/bin/2d/unet2d_binary"
TRAIN_IMG_PATH = "/content/drive/My Drive/cs8395_deep_learning/assignment3/data/Train/img_2d/"
TRAIN_LABEL_PATH = "/content/drive/My Drive/cs8395_deep_learning/assignment3/data/Train/label_filtered_2d/"
VAL_IMG_PATH = "/content/drive/My Drive/cs8395_deep_learning/assignment3/data/Val/img_2d/"
VAL_LABEL_PATH = "/content/drive/My Drive/cs8395_deep_learning/assignment3/data/Val/label_filtered_2d/"
...
def test(model, device, test_loader, test_losses):
    ...
    with torch.no_grad():
        ...
        for index, (data, target) in enumerate(test_loader):
            ...
            # Get the prediction by getting the index with the maximum probability and convert
            # it and the target to numpy array
            pred = output.argmax(dim=1, keepdim=True).cpu().numpy()
            target_np = target.cpu().numpy()

            # Calculate the true positives, false positives, true negatives, and false negatives
            # and increment total sums
            true_positives = float(np.sum(np.where(np.logical_and(pred == 1, target_np == 1), 1, 0)))
            false_positives = float(np.sum(np.where(np.logical_and(pred == 1, target_np == 0), 1, 0)))
            true_negatives = float(np.sum(np.where(np.logical_and(pred == 0, target_np == 0), 1, 0)))
            false_negatives = float(np.sum(np.where(np.logical_and(pred == 0, target_np == 1), 1, 0)))
